import directus, {type TattooStyle} from './directus';
...
export async function getTattoosByStyleAsync(style: string): Promise<GallerySlide[]> {
    const cmsTattoos = await directus.request(
        readItems("Tattoos", {
            filter: {
                Style: {
...
export async function getTattooStylesAsync(): Promise<{ Style: string; Image: string }[]> {
