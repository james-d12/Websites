...
export function GalleryGrid(props: { slides: GallerySlide[] }) {
  ...
  const filteredSlides = createMemo(() =>
    props.slides.filter(
      (slide) =>
        !selectedCategory() ||
        (slide.category ?? "Uncategorized") === selectedCategory(),
    ),
  );
    ...
    <>
      ...
      <Show when={isFullscreen()}>
        ...
        >
          ...
          {/* Navigation controls */}
          <div class="flex justify-between w-full max-w-[90vw] mt-4 md:absolute md:top-1/2 md:left-0 md:right-0 md:mt-0 md:justify-between md:px-4">
            <button
              onClick={(e) => {
                e.stopPropagation();
                prevSlide();
