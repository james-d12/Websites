...
const TOTAL_DAYS = Math.round((WAR_END.getTime() - WAR_START.getTime()) / 86_400_000);

const CARTO_DARK = 'https://basemaps.cartocdn.com/gl/voyager-gl-style/style.json';
...
export default function WW2Map({ flags }: { flags: Record<string, string> }) {
  ...
  const isEventVisible = useCallback((ev: WW2Event) => {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {/* Single faction */}
                  {f1 && !f2 && (img1
                    ? <img src={img1} alt={f1} className="absolute inset-0 w-full h-full object-cover" />
                    : <span style={{ fontSize: flagSize }}>{COUNTRY_FLAGS[f1]}</span>
                  )}

                  {/* Dual faction: each half fills its side */}
                  {f1 && f2 && (
                    <>
                      <div className="absolute left-0 top-0 w-1/2 h-full overflow-hidden" style={{ background: c1 }}>
                        {img1
                          ? <img src={img1} alt={f1} className="w-full h-full object-cover" />
                          : <span className="absolute inset-0 flex items-center justify-center" style={{ fontSize: flagSize }}>{COUNTRY_FLAGS[f1]}</span>
                        }
                      </div>
                      <div className="absolute right-0 top-0 w-1/2 h-full overflow-hidden" style={{ background: c2 ?? undefined }}>
                        {img2
                          ? <img src={img2} alt={f2} className="w-full h-full object-cover" />
                          : <span className="absolute inset-0 flex items-center justify-center" style={{ fontSize: flagSize }}>{COUNTRY_FLAGS[f2]}</span>
                        }
                      </div>
                    </>
                  )}
